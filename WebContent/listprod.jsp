...
=======
    ...
    <%
    // Get the product name to search for from the request
    String name = request.getParameter("productName");
    ...
    // SQL query to fetch products based on search term
    try (Connection con = DriverManager.getConnection(url, uid, pw)) {
        String SQL = "SELECT * FROM product WHERE productName LIKE ?";
        PreparedStatement pstmt = con.prepareStatement(SQL);
        ...
        if (name == null || name.trim().isEmpty()) {
            ...
            pstmt.setString(1, "%" + name + "%"); // Search for products matching the name
        }
    ...
    <table border="2">
        ...
        <%
        // Iterate through the result set and display each product in the table
        while (rst.next()) {
            String productId = rst.getString("productId");
            String productName = rst.getString("productName");
            double productPrice = rst.getDouble("productPrice");

            // URL encode the product name for safe use in URLs
            String productPage = "product.jsp?id=" + productId + "&name=" + URLEncoder.encode(productName, "UTF-8") + "&price=" + productPrice;
            String addToCartLink = "addcart.jsp?id=" + productId + "&name=" + URLEncoder.encode(productName, "UTF-8") + "&price=" + productPrice;
        %>
        <tr>
